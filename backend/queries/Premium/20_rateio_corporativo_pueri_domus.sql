-- @id: premium_rateio_corporativo_pueri_domus
-- @name: Premium - Rateio Corporativo YTD - Pueri Domus
-- @category: Premium
-- @order: 20
...
itens AS (
    SELECT
        ...
        , 1)                                                                                                                                  AS `Var_pp_FcstR`,
        -- "Rateio Corporativo Gestão e Gente" não existe na Premium consolidado
        -- (query 10) — aparece no print Pueri Domus, então incluída aqui
        CASE m.Nome_Conta
            ...
            WHEN 'Rateio Corporativo Gestão e Gente'  THEN 5
            ELSE 50
        END AS sort_order
    FROM metricas m
    CROSS JOIN rol r
...
WHERE m.Nome_Conta IN (
    ...
    'Rateio Diretoria Executiva', 'Rateio TI', 'Rateio Corporativo Gestão e Gente'
)
