-- @id: premium_principais_metricas_pueri_domus_verbo
...
-- Var % vs Bgt/Fcst = razão (atual/comparação*100, 100% = igual) — confirmado com o print
-- da unidade Verbo. Var % vs 25 continua variação percentual normal (0% = igual).
select 'Alunos #' as `YTD`,
    ...
    round(case when alunos_26b > 0 then alunos_26r / alunos_26b * 100 else null end, 1)                      as `Var %|26 x Bgt`,
    round(alunos_26r - alunos_26f, 0)                                                                        as `Var #|26 x Fcst`,
    round(case when alunos_26f > 0 then alunos_26r / alunos_26f * 100 else null end, 1)                      as `Var %|26 x Fcst`,
