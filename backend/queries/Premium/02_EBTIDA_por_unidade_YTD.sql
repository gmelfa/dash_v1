...
base_dados AS (
    SELECT
        ...
        CASE
            ...
            WHEN f.Nome_Unidade = 'Pueri Domus - CSC Local'               THEN 6
            WHEN f.Nome_Unidade = 'Diretoria Premium - Pueri Domus'       THEN 7
            ...
            WHEN f.Nome_Unidade = 'C. Patrício - CSC Local'               THEN 14
...
metricas_agregadas AS (
    SELECT
        ...
        END AS alu_26r,
        SUM(CASE WHEN ano = ano_anterior AND Origem IN ('Resultado','Ajustes') AND rol_flag = 1 THEN Valor * -1 ELSE 0 END) / 1000                             AS rol_25r,
        SUM(CASE WHEN ano = ano_atual    AND Origem = 'Budget'                 AND rol_flag = 1 THEN Valor * -1 ELSE 0 END) / 1000                             AS rol_26b,
        SUM(CASE WHEN ano = ano_atual    AND Origem = 'Forecast'               AND rol_flag = 1 THEN Valor * -1 ELSE 0 END) / 1000                             AS rol_26f,
        SUM(CASE WHEN ano = ano_atual    AND Origem IN ('Resultado','Ajustes') AND rol_flag = 1 THEN Valor * -1 ELSE 0 END) / 1000                             AS rol_26r,
...
uniao_final AS (
    ...
    SELECT 'Pueri Domus', 8, 2, SUM(alu_25r), SUM(rol_25r), SUM(ebt_25r), SUM(alu_26b), SUM(rol_26b), SUM(ebt_26b), SUM(alu_26f), SUM(rol_26f), SUM(ebt_26f), SUM(alu_26r), SUM(rol_26r), SUM(ebt_26r)
    FROM metricas_agregadas WHERE Grupo = 'Pueri Domus' AND tipo_operacao = 'Operações Mantidas'

    UNION ALL

    -- Subtotal Carolina Patrício
    SELECT 'C. Patrício', 15, 2, SUM(alu_25r), SUM(rol_25r), SUM(ebt_25r), SUM(alu_26b), SUM(rol_26b), SUM(ebt_26b), SUM(alu_26f), SUM(rol_26f), SUM(ebt_26f), SUM(alu_26r), SUM(rol_26r), SUM(ebt_26r)
    FROM metricas_agregadas WHERE Grupo = 'Carolina Patrício'

    UNION ALL

    -- Subtotal Sphere
    SELECT 'Sphere International', 21, 2, SUM(alu_25r), SUM(rol_25r), SUM(ebt_25r), SUM(alu_26b), SUM(rol_26b), SUM(ebt_26b), SUM(alu_26f), SUM(rol_26f), SUM(ebt_26f), SUM(alu_26r), SUM(rol_26r), SUM(ebt_26r)
    FROM metricas_agregadas WHERE Grupo = 'Sphere'

    UNION ALL

    -- Operações Mantidas (Total)
    SELECT 'Operações Mantidas', 25, 3, SUM(alu_25r), SUM(rol_25r), SUM(ebt_25r), SUM(alu_26b), SUM(rol_26b), SUM(ebt_26b), SUM(alu_26f), SUM(rol_26f), SUM(ebt_26f), SUM(alu_26r), SUM(rol_26r), SUM(ebt_26r)
    FROM metricas_agregadas WHERE tipo_operacao = 'Operações Mantidas'

    UNION ALL

    -- Novos Negócios (Total)
    SELECT 'Novos Negócios', 35, 3, SUM(alu_25r), SUM(rol_25r), SUM(ebt_25r), SUM(alu_26b), SUM(rol_26b), SUM(ebt_26b), SUM(alu_26f), SUM(rol_26f), SUM(ebt_26f), SUM(alu_26r), SUM(rol_26r), SUM(ebt_26r)
    FROM metricas_agregadas WHERE tipo_operacao = 'Novos Negócios'

    UNION ALL

    -- Total Premium
    SELECT 'Total Premium', 40, 4, SUM(alu_25r), SUM(rol_25r), SUM(ebt_25r), SUM(alu_26b), SUM(rol_26b), SUM(ebt_26b), SUM(alu_26f), SUM(rol_26f), SUM(ebt_26f), SUM(alu_26r), SUM(rol_26r), SUM(ebt_26r)
    FROM metricas_agregadas
)
...
SELECT
    ...
    ROUND(alu_26r - alu_26f, 0)                                               AS `Var_Alu_Abs`,
    ROUND(CASE WHEN alu_26f > 0 THEN (alu_26r / alu_26f - 1) * 100 ELSE 0 END, 1) AS `Var_Alu_Perc`
FROM uniao_final
